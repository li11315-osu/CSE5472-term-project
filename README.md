...
Ultimately, either side having enough of a resource advantage will allow them to always come out on top regardless how much their adversary tries to exploit the properties of the protocol, but the threshold for "enough" can be made more elusive if one plays their cards well. In this project, we try to quantify just how much of a difference it can make, if any, to try having your defenses work smarter when they can't work any harder.
